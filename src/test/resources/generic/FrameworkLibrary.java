 ...
 * ######################################################################################################
 */
	public static void updateLogStep(boolean stepStatus)
	{
	    if(stepStatus)
		{
	    	if(plog.length()>0)
					...
					if (ExcelIteration == CurrentIteration)
					{
						...
						for(int i = 2;i<colcount;i++)
						{
							...
							if (cell!=null && ColumnValue1.contains(":="))
							{
								...
								putData(arrvalues[0].trim(), arrvalues[1].trim());
							}
						}
						foundrow = 1;
						break;
				...
				{		
					...
					if (ExcelIteration == CurrentIteration)
					{
						...
						for(int i = 2;i<colcount;i++)
						{
							...
							String CurrentValue =getData(ColumnValues[0]);
 ...
 * ######################################################################################################
 */
	public static String getData(String VairableName)
    {
		String Value = "";
    	try
	    {
    		VairableName = VairableName.toLowerCase();
    		Value = tdmap.get(VairableName);
 ...
 * ######################################################################################################
 */
		public static void putData(String VairableName,String value)
	    {
	    	try
		    {
	    		VairableName = VairableName.toLowerCase();
	    		tdmap.put(VairableName, value);
		    } catch (Exception ignore)
		    {
 ...
 */
	...
	{
		takescreenshotforlogger = false;
		takescreenshotForFailStatements = false;
		takescreenshotForpassstatements = false;
	}
/* ######################################################################################################
 * Method Name: disableTakingScreenshots
 * Description: disable Highlight for each action.
